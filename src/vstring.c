...
char* v_strcpy(char* to, const char* from)
{
    const char* src;
    char* dst;
...
char* v_strncpy(char* to, const char* from, u16 len)
{
    const char* src;
    char* dst;
    u16 i;
...
char* v_strcat(char* to, const char* from)
{
    const char* src;
    char* dst;
...
{
    if (value < -500000000) {
        v_strcpy(str, "<-500000000");
        return 10;
    }

    if (value < 0) {
        *str = '-';
        return v_uintToStr(-value, str + 1, minsize);
    } else
        return v_uintToStr(value, str, minsize);
}
...
u16 v_uintToStr(u32 value, char* str, u16 minsize)
{
    ...
    if (value > 10000) {
        ...
        len = uint16ToStr(value, str, minsize);
...
void v_fix32ToStr(fix32 value, char* str, u16 numdec)
{
    ...
    dst += v_uintToStr(fix32ToInt(v), dst, 1);
    *dst++ = '.';
