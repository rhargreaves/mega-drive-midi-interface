...
{
    if (d.rx.cb) {
        d.rx.cb(LSD_STAT_COMPLETE, d.rx.ch, d.rx.buf, d.rx.pos, d.rx.ctx);
    }
}
...
static void process_recv(void)
{
    ...
    case LSD_RECV_CH_LENH: // Receive CH and len high
        ...
        if (!(LSD_STX_ETX == recv)) {
            ...
            if (d.rx.ch >= LSD_MAX_CH || !d.ch_enable[d.rx.ch]) {
...
void lsd_process(void)
{
    ...
    do {
        ...
        if (d.tx.stat > LSD_SEND_IDLE && uart_tx_ready()) {
            active = TRUE;
            for (int i = 0; i < UART_TX_FIFO_LEN && d.tx.stat > LSD_SEND_IDLE;
                 i++) {
                process_send();
            }
        }
...
enum lsd_status lsd_send_sync(uint8_t ch, const char* data, int16_t len)
{
    ...
    while (d.tx.stat > LSD_SEND_IDLE) { lsd_process(); }
...
enum lsd_status lsd_recv_sync(char* buf, uint16_t* len, uint8_t* ch)
{
    ...
    while (d.rx.stat > LSD_RECV_IDLE) { lsd_process(); }
